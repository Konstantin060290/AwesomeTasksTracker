...
public class AuthenticateAnalyticConsumer : IAuthenticateAnalyticConsumer
{
    public AuthenticateAnalyticConsumer()
    {
        ConsumeAuthenticate();
    }
    ...
    private async void Listen()
    {
        ...
        builder.Subscribe(KafkaTopicNames.AnalyticAuthRequests);
        ...
        CancellationTokenSource cts)
    {
        if (Uri.TryCreate(new Uri("http://localhost:5154", UriKind.Absolute),
                $"AuthenticateBroker/AuthenticateUserInAnalytic", out var uri))
        {
...
public interface IAuthenticateAnalyticConsumer
{
