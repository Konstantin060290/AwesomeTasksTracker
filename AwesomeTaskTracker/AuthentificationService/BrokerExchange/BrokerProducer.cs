...
public class BrokerProducer
{
    ...
    public async Task SendAuthAnalyticConfirm(string userEmail, string eventName)
    {
        var authConfirmMessage = new AuthConfirm
        {
            UserEmail = userEmail
        ...
        await producerBuilder.ProduceAsync(KafkaTopicNames.AnalyticAuthAnswers, new Message<string, string>
        {
            Key = eventName,
            Value = message
        });
